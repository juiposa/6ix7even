...
namespace _6ix7even.Signature
{
    ...
    internal class SigLocator
    {
        ...
        {
            _state = state;
            nint baseaddr = _state.SigScanner.Module.BaseAddress;
        ...
        {
            if (_state.SigScanner.TryScanText(signature, out nint addr) == true)
            {
                return addr;
            }
            return IntPtr.Zero;
        ...
        {
            if (_state.SigScanner.TryScanData(signature, out nint addr) == true)
            {
                return addr;
            }
            return IntPtr.Zero;
        ...
        {
            if (_state.SigScanner.TryScanModule(signature, out nint addr) == true)
            {
                return addr;
            }
            return IntPtr.Zero;
        ...
        {            
            if (_state.SigScanner.TryGetStaticAddressFromSig(signature, out nint addr, offset) == true)
            {
                return addr;
            }
            return IntPtr.Zero;
        ...
        public nint OldGetStaticAddressFromSig(string signature, int offset = 0)
        {
            ...
            nint bAddr = _state.SigScanner.Module.BaseAddress;
            long num;
            do
            {
            ...
            while (!(num >= _state.SigScanner.DataSectionOffset && num <= _state.SigScanner.DataSectionOffset + _state.SigScanner.DataSectionSize));
        ...
        private IntPtr OldScanText(string pattern)
        {
            ...
            if (results.Count != 1)
            {
                Log.Error("Signature pattern returned {0} hits", results.Count);
                return 0;
            }
